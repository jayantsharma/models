...
def main(_):
  if not (FLAGS.train_dataset_dir and FLAGS.test_dataset_dir):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      test_label -= FLAGS.labels_offset

      train_image_size = FLAGS.train_image_size or network_fn.default_image_size
      test_image_size = train_image_size

      train_means = pickle.load(open("{}/data_list.pkl".format(FLAGS.train_dataset_dir), 'rb'))['means']
      train_image = image_preprocessing_fn(train_image, train_image_size, train_image_size, means=train_means)
      ...
      train_domain_labels = slim.one_hot_encoding(train_domain_labels, 2)
      test_domain_labels = slim.one_hot_encoding(test_domain_labels, 2)

      train_batch_queue = slim.prefetch_queue.prefetch_queue(
          [train_images, train_domain_labels], capacity=2 * deploy_config.num_clones)
      test_batch_queue = slim.prefetch_queue.prefetch_queue(
          [test_images, test_domain_labels], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(train_batch_queue, test_batch_queue):
      ...
      slim.losses.softmax_cross_entropy(
          train_domain_logits, train_domain_labels, label_smoothing=FLAGS.label_smoothing, weights=1.0)
      slim.losses.softmax_cross_entropy(
          test_domain_logits, test_domain_labels, label_smoothing=FLAGS.label_smoothing, weights=1.0)
